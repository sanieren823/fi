...
// 			return self;
// 		}
// 		let n: FiLong = self << self.last().unwrap().leading_zeros() as usize;
// 		let inverse: FiLong = n.reverse_bits();
// 		let mut q = FiLong{sign: sign, value: Vec::with_capacity(self.value.capacity())};
// 		let mut r: FiLong = FiLong{sign: false, value: vec![0]};
// 		for i in (0..self.len()).rev() {
// 			r <<= 1;
// 			r &= FiLong{sign: false, value: vec![1]};
// 			if r >= other {
// 				r -= other;
// 				q |= FiLong{sign: false, value: vec![1]} << i;
// 			}
